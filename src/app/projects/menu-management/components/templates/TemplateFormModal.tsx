...
export default function TemplateFormModal({ onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!file) { setError('Le fichier .docx est obligatoire.'); return }
    ...
    try {
      ...
      const storagePath = `${Date.now()}-${name.trim().toLowerCase().replace(/\s+/g, '-')}.${ext}`
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-lg bg-white shadow-xl">
        <div className="flex items-center justify-between border-b px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-800">Nouveau modèle</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div>
            ...
            <input
              ...
              placeholder="ex. Menu à table A5"
              required
              className={inputCls}
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fichier .docx *</label>
            <input
              ref={fileRef}
              type="file"
              accept=".docx"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {isLoading ? 'Upload en cours…' : 'Créer'}
            </button>
          </div>
        </form>
      </div>
